module AuthenticatedTestHelper
  ...
  def login_as(which_user=:standard_user, load_account=false)
    if which_user.nil? # logout
      @request.session[:user] = nil
      return nil
    end
    user = users(which_user)
    @request.session[:user] = user.id

    stub_authenticate_password do
      @current_user = User.authenticate(user.login, 'ignored')
      account_setup_user if load_account
    end
    return current_user
  end
...
private
  def stub_authenticate_password
    old_auth_password = User.instance_method(:authenticate_password)
    User.send(:define_method, :authenticate_password) { true }
    begin
      return yield
    ensure
      User.send(:define_method, :authenticate_password, old_auth_password)
    end
  end
end
